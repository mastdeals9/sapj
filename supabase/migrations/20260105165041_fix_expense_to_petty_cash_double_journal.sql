/*
  # Fix Expense to Petty Cash - Prevent Double Journal Entries

  ## Problem
  When moving an expense from finance_expenses to petty cash using `move_expense_to_petty_cash()`:
  1. The expense already has a journal entry created by `auto_post_expense_accounting`
  2. Creating a petty_cash_transaction triggers `post_petty_cash_to_journal()`
  3. This creates a SECOND journal entry, causing imbalance and constraint violations

  ## Solution
  Modify the petty cash trigger to skip journal entry creation when:
  - The transaction has `source='moved_from_tracker'` (moved from expense tracker)
  - The transaction has a `finance_expense_id` link (already has journal entry)

  ## Changes
  1. Update `post_petty_cash_to_journal()` to check for existing journal entries
  2. Skip journal creation if expense was moved from tracker
*/
...
DECLARE
  ...
  v_line_num INT;
BEGIN
  -- CRITICAL: Skip journal creation if this is a moved expense
  -- The original expense already has a journal entry
  IF NEW.source = 'moved_from_tracker' OR NEW.finance_expense_id IS NOT NULL THEN
    RETURN NEW;
  END IF;
...
IS 'Posts petty cash transactions to journal entries. Skips journal creation for expenses moved from tracker (already have journal entries).';