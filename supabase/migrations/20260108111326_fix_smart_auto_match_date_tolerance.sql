/*
  # Fix Auto-Match Date Tolerance - Smarter Matching

  ## Problem
  Auto-match was matching transactions with expenses that are too far apart in time
  (e.g., December transaction to October expense - 60+ days apart)

  ## Solution
  1. Enforce strict 7-day date tolerance
  2. Ensure expenses are not already matched to other bank statements
  3. Match by bank_account_id if available for higher accuracy
  4. Prioritize: exact amount + same date > close amount + close date
  5. Add paid_by filter to only match bank-paid expenses

  ## Changes
  - Drop old auto-match functions
  - Create new improved auto_match_smart() function with:
    * Maximum 7-day date window (±7 days from transaction date)
    * Check expense is not already reconciled
    * Match bank_account_id when available
    * Better scoring algorithm
*/

-- Drop old functions
DROP FUNCTION IF EXISTS auto_match_all_bank_transactions();
DROP FUNCTION IF EXISTS auto_match_with_memory();

-- Create improved smart auto-match function
CREATE OR REPLACE FUNCTION auto_match_smart()
RETURNS TABLE(matched_count int, suggested_count int, skipped_count int) AS $$
DECLARE
