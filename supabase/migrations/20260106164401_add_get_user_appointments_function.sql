/*
  # Add Function to Get User Appointments
  
  1. Function
    - Gets all appointments where user is creator OR participant
    - Returns in format needed for calendar display
...
BEGIN
  ...
  WHERE ca.activity_type IN ('meeting', 'video_call', 'phone_call')
    ...
    AND (p_start_date IS NULL OR ca.follow_up_date >= p_start_date)
    AND (p_end_date IS NULL OR ca.follow_up_date <= p_end_date)
