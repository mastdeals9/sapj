/*
  # Fix Petty Cash Double Journal Entry - Complete Fix

  ## Root Cause
  There are TWO scenarios causing double journal entries:
  
  1. When creating expense with payment_method='petty_cash':
     - auto_post_expense_accounting creates journal entry
     - It then creates petty_cash_transaction with source='finance_expense'
     - This triggers post_petty_cash_to_journal which creates ANOTHER journal entry
  
  2. When moving expense to petty cash:
     - Expense already has journal entry
     - move_expense_to_petty_cash creates petty_cash_transaction with source='moved_from_tracker'
     - This triggers post_petty_cash_to_journal which creates ANOTHER journal entry
  
  ## Solution
  Update post_petty_cash_to_journal to skip journal creation when:
  - source = 'finance_expense' (already has journal from expense trigger)
  - source = 'moved_from_tracker' (already has journal from original expense)
  - finance_expense_id IS NOT NULL (linked to existing expense with journal)
*/
...
BEGIN
  ...
  IF NEW.source IN ('moved_from_tracker', 'finance_expense') OR NEW.finance_expense_id IS NOT NULL THEN
    RETURN NEW;
  END IF;
  ...
  ELSIF NEW.transaction_type = 'expense' THEN
    ...
    -- Only for standalone petty cash expenses (not linked to finance_expenses)
...
IS 'Posts petty cash transactions to journal entries. Skips journal creation for transactions linked to finance_expenses (which already have their own journal entries from expense triggers).';